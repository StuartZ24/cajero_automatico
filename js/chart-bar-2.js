var ctx = document.getElementById("myBarChart2");           //Obtenemos una referencia al elemento canvas donde dibujaremos el grafico
...
for (let i = 0; i < array.length; i++) {//Recorremos el arreglo de tipo retiro
    color1.push(color());//Luego crearemos los colores de acuerdo a la cantidad de retiros hechos
...
var myChart = new Chart(ctx, {                      //Funcion para generar un grafico en el elemento canvas
  ...
  data: {
      ...
      datasets: [{                                  // Es un conjunto de datos donde agregamos la data o datos
          label: 'Cantidad de Retiro en USD',       // Es la leyenda y la información sobre el grafico.
